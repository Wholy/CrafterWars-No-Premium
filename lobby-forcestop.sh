...
while [ $CountdownTime -gt 0 ]; do
  if [ $CountdownTime -eq 1 ]; then
    screen -Rd lobby -X stuff "say Stopping server in 60 seconds...$(printf '\r')"
    echo "60 seconds"
    sleep 30;
    screen -Rd lobby -X stuff "say Stopping server in 30 seconds...$(printf '\r')"
    echo "30 seconds"
    sleep 20;
    screen -Rd lobby -X stuff "say Stopping server in 10 seconds...$(printf '\r')"
    echo "10 seconds"
    sleep 10;
    break
  fi
